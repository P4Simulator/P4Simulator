 ...
 */
#ifndef SLB_NET_DEVICE_H
#define SLB_NET_DEVICE_H
 ...
 *
 * The SLBNetDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a SLBNetDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class SLBNetDevice : public BridgeNetDevice {
public:
  /**
  ...
  SLBNetDevice ();
  virtual ~SLBNetDevice ();

  /**
   * \brief Add a 'port' to a bridge device
   * \param bridgePort the NetDevice to add
   *
   * This method adds a new bridge port to a SLBNetDevice, so that
   ...
   * bridging node you must enable IP on the SLBNetDevice itself,
...
//    */
//   SLBNetDevice (const SLBNetDevice &);
...
//    * \returns
//    */
//   SLBNetDevice &operator = (const SLBNetDevice &);
