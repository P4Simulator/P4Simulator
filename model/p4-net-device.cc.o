...
P4NetDevice::P4NetDevice() :
    ...
    char * a3;
    std::string jsonPath;
    if(g_networkFunc==FIREWALL) 
       {
	     jsonPath=g_nfDir+"firewall/firewall.json";
         a3=(char*)jsonPath.data();

	   }
    if(g_networkFunc==SILKROAD)
       {
	    jsonPath=g_nfDir+"silkroad/silkroad.json";
        a3=(char*)jsonPath.data();
	}
    if(g_networkFunc==ROUTER)
       {
	    jsonPath=g_nfDir+"router/router.json";
        a3=(char*)jsonPath.data();
	}
    if(g_networkFunc==SIMPLE_ROUTER)
       {
        jsonPath=g_nfDir+"simple_router/simple_router.json";
		a3=(char*)jsonPath.data();
	}
    char * args[2] = { NULL,a3};
    p4Model->init(2, args);
...
int P4Model::init(int argc, char *argv[]) {
    ...
    if(g_populateFlowtableWay==LOCAL_CALL)
    {
        this->InitFromCommandLineOptionsLocal(argc, argv, m_argParser);
        ReadTableActionMatchType(g_p4MatchTypePath,flowtableMatchType);
        PopulateFlowTable(g_flowtablePath);
    ...
    else
    {
        ...
        if(g_populateFlowtableWay==RUNTIME_CLI)
        {
