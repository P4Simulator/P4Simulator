...
// TO DO: attain meter, counter attribute
		...
		try {
			...
			case TABLE_NUM_ENTRIES: { //table_num_entries <table name>
				try {
					...
					{
						size_t num_entries;
						if (mt_get_num_entries(0, parms[1], &num_entries) != bm::MatchErrorCode::SUCCESS)
			...
			case TABLE_CLEAR: {// table_clear <table name>
				try {
					if (parms.size() == 2)
					{
						if (mt_clear_entries(0, parms[1], false) !=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
					...
					if (parms.size() == 3)
					{
						if (m_meter.count(parms[1]) > 0)
						{
							if (m_meter[parms[1]].isDirect)//direct
							{
								...
								if (mt_get_meter_rates(0, parms[1], handle, &configs) != bm::MatchErrorCode::SUCCESS)
									throw P4Exception(NO_SUCCESS);
							...
							else//indirect
							{
								...
								std::vector<bm::Meter::rate_config_t> configs;
								if (meter_get_rates(0, parms[1], idx, &configs) != 0)
									throw P4Exception(NO_SUCCESS);
								for (size_t i = 0; i < configs.size(); i++)
								{
									std::cout << "info_rate:" << configs[i].info_rate << " burst_size:" << configs[i].burst_size << std::endl;
					...
					if (parms.size() == 3)
					{
						if (m_counter.count(parms[1]) > 0)
						{
							if (m_counter[parms[1]].isDirect)//direct
							{
								...
								if (mt_read_counters(0, parms[1], handle, &bytes, &packets) != bm::MatchErrorCode::SUCCESS)
									throw P4Exception(NO_SUCCESS);
							...
							else
							{
								...
								bm::MatchTableAbstract::counter_value_t bytes;
								bm::MatchTableAbstract::counter_value_t packets;
								if(read_counters(0, parms[1], index, &bytes, &packets)!=0)
									throw P4Exception(NO_SUCCESS);
				...
				try {
					if (parms.size() == 2)
					{
						if (m_counter.count(parms[1]) > 0)
						{
							if (m_counter[parms[1]].isDirect)//direct
							{
								if(mt_reset_counters(0, m_counter[parms[1]].tableName)!=bm::MatchErrorCode::SUCCESS)
									throw P4Exception(NO_SUCCESS);
							}
							else //indirect
							{
								if(reset_counters(0, parms[1])!=0)
									throw P4Exception(NO_SUCCESS);
							}
						}
						else
			...
			case REGISTER_READ: {//register_read <name> [index]
				try {
					if (parms.size() == 3)
					{
						size_t index(StrToInt(parms[2]));
						bm::Data value;
						if(register_read(0, parms[1], index, &value)!=0)
							throw P4Exception(NO_SUCCESS);
			...
			case REGISTER_WRITE: { //register_write <name> <index> <value>
				try {
					if (parms.size() == 4)
					{
						...
						if(register_write(0, parms[1], index, value)!=0)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
			...
			case TABLE_DUMP_ENTRY: {// table_dump_entry <table name> <entry handle>
				try {
					if (parms.size() == 3)
					{
						...
						if(mt_get_entry(0, parms[1], handle, &entry)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
		...
		try {
			...
			case TABLE_SET_DEFAULT: {//table_set_default <table name> <action name> <action parameters>
				try {
					if (parms.size() >= 3) {
						...
						if(mt_set_default_action(0, parms[1], parms[2], actionData)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
			...
			case TABLE_ADD: {//table_add <table name> <action name> <match fields> => <action parameters> [priority]
				try {
					...
					if (matchType != bm::MatchKeyParam::Type::TERNARY&&matchType != bm::MatchKeyParam::Type::RANGE)
					{
						...
						if(mt_add_entry(0, parms[1], matchKey, parms[2], actionData, &handle, priority)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
					{
			...
			case TABLE_SET_TIMEOUT: { //table_set_timeout <table_name> <entry handle> <timeout (ms)>
				try {
					if (parms.size() == 4)
					{
						...
						if(mt_set_entry_ttl(0, parms[1], handle, ttl_ms)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
			...
			case TABLE_MODIFY: {//table_modify <table name> <action name> <entry handle> [action parameters]
				try {
					if (parms.size() >= 4) {
						...
						//TO DO:judge action_data_num equal action need num
						if(mt_modify_entry(0, parms[1], handle, parms[2], actionData)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
			...
			case TABLE_DELETE: {// table_delete <table name> <entry handle>
				try {
					...
					{
						bm::entry_handle_t handle(StrToInt(parms[2]));
						if(mt_delete_entry(0, parms[1], handle)!=bm::MatchErrorCode::SUCCESS)
							throw P4Exception(NO_SUCCESS);
					}
					else
						throw P4Exception(PARAMETER_NUM_ERROR);
