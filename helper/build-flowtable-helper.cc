		...
		for (unsigned int i = 0; i < m_podNum; i++)// number pod
		{
			for (unsigned int j = 0; j < halfPodNum; j++)
			{
				...
				for (size_t p = 0; p < m_switchNodes[curEdgeSwitchIndex].portNode.size(); p++)
				{
					...
					{
						//attain host ipAddr
						curEdgeSwitchLinkHostIndex.insert(m_switchNodes[curEdgeSwitchIndex].portNode[p].nodeIndex);
						switchReachHostIndexMap[curEdgeSwitchIndex].push_back(m_switchNodes[curEdgeSwitchIndex].portNode[p].nodeIndex);
						std::string dstIp = m_hostNodes[m_switchNodes[curEdgeSwitchIndex].portNode[p].nodeIndex].ipAddr;
						m_switchNodes[curEdgeSwitchIndex].flowTableEntries.push_back(FlowTableEntry_t("", dstIp, UintToPortStr(p)));
					}
					else
				...
				for (unsigned int p = 0; p < hostNum; p++)
				{
					if (curEdgeSwitchLinkHostIndex.count(p) == 0)
					{
						...
						m_switchNodes[curEdgeSwitchIndex].flowTableEntries.push_back(FlowTableEntry_t("", dstIp, UintToPortStr(transferPort)));
					}
				}
			}
		}
			...
			for (unsigned int j = 0; j < halfPodNum; j++)
			{
				...
				for (size_t p = 0; p < m_switchNodes[curAggrSwitchIndex].portNode.size(); p++)
				{
					if (m_switchNodes[curAggrSwitchIndex].portNode[p].nodeIndex > curAggrSwitchIndex)//mean link edge switch
					{
						...
						for (size_t q = 0; q < switchReachHostIndexMap[linkSIndex].size(); q++)
						{
							...
							m_switchNodes[curAggrSwitchIndex].flowTableEntries.push_back(FlowTableEntry_t("", dstIp, UintToPortStr(p)));
				...
				for (unsigned int p = 0; p < hostNum; p++)
				{
					if (curAggrSwitchLinkHostIndex.count(p) == 0)
					{
						...
						//random select a out port
						unsigned int transferPort = otherPortIndex[rand() % otherPortIndex.size()];
						m_switchNodes[curAggrSwitchIndex].flowTableEntries.push_back(FlowTableEntry_t("", dstIp, UintToPortStr(transferPort)));
					}
				}
			}
		}
			...
			for (size_t p = 0; p < m_switchNodes[i].portNode.size(); p++)
			{
				if (m_switchNodes[i].portNode[p].flag == 1)//represent switch
				{
					...
					for (size_t q = 0; q < switchReachHostIndexMap[linkSIndex].size(); q++)
					{
						unsigned int linkTIndex = switchReachHostIndexMap[linkSIndex][q];
						std::string dstIp = m_hostNodes[linkTIndex].ipAddr;
						m_switchNodes[i].flowTableEntries.push_back(FlowTableEntry_t("", dstIp, UintToPortStr(p)));
					}
				}
			}
		}
