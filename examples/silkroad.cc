...
int main(int argc, char *argv[]) {
	...
	unsigned int coreNum =(unsigned int)((treeLevel / 2)*(treeLevel / 2));
	unsigned int hostNum =(unsigned int)(treeLevel*treeLevel*treeLevel / 4);
	terminals.Create(coreNum);
	std::cout<<"Add outer terminal, terminal Num:"<<terminals.GetN()<<std::endl;
	for (unsigned int p = 0; p < coreNum; p++)
	{
		NetDeviceContainer link = csma.Install(NodeContainer(terminals.Get(p+hostNum),csmaSwitch.Get(p)));
		terminalDevices[p].Add(link.Get(0));
		terminalNodeLinkedSwitchIndex.push_back(p);
		terminalNodeLinkedSwitchPort.push_back(switchDevices[p].GetN());
		terminalNodeLinkedSwitchNumber.push_back(terminalDevices[p].GetN());

		switchPortInfo[p].push_back("t" + int_to_str(p + hostNum));
		switchDevices[p].Add(link.Get(1));
	}
	...
	//assign ip to host devices
	for (unsigned int k = coreNum; k<switchNum; k++)
	{
	...
	//**********************assign ip to outer terminal device**************************************************
	//ipv4.SetBase("10.1.0.0", "255.255.0.0");
	for (unsigned int p = 0; p < coreNum; p++)
	{
